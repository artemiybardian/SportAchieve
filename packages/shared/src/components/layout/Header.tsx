...
export function Header({ name, avatarUrl, daysLeft, hasSubscription }: HeaderProps) {
  ...
  const isMiniAppChrome = platform?.name === 'vk' || platform?.name === 'max';
  ...
  return (
    <header className="header-gradient flex items-center gap-2 px-4 sm:px-5 py-4 rounded-b-3xl shadow-lg min-w-0">
      ...
      {/* Резерв под системные кнопки мини-приложения (VK / MAX), чтобы тема не упиралась в оверлей */}
      {isMiniAppChrome ? <div className="h-9 w-[4.5rem] shrink-0" aria-hidden /> : null}
    </header>
  );
}
